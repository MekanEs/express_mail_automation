...
import { report, DashboardMetrics } from '../types/types'; // Импортируем нужные типы
...
class DashboardController {
    public async getDashboardMetrics(_req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!reportStats) {
                res.status(200).send({
                    summary: { totalReports: 0, totalEmailsFound: 0, totalEmailsProcessed: 0, successRate: 0 },
                    recentProcesses: [],
                    accountsStats: {}
                });
                return;
            ...
            const totalEmailsProcessed = reportStats.reduce((sum, item) => sum + (item.emails_processed || 0), 0);
            ...
            const accountsData: Record<string, { total: number; success: number; failure: number, partial: number }> = {};
            ...
            const uniqueProcessIds = [...new Map(reportStats.map(item => [item.process_id, item])).values()]
                                    ...
                                    .map(item => ({ process_id: item.process_id as string, created_at: item.created_at as string }));
